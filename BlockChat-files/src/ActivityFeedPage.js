...
const ActivityFeedPage = () => {
    ...
    const { feedItems, setFeedItems, user, isVip, isAdmin, featureToggles } = useOutletContext();
    ...
    const handlePostUpdate = async (content) => {
        ...
        const postedItem = await mockService.addActivityFeedItem(newPost);
    ...
    const handleLike = async (itemId) => {
        ...
        try {
            await mockService.updateActivityFeedItem(itemId, { likes: item.likes + 1 });
        } catch (error) {
    ...
    const handleAddComment = async (itemId, content) => {
        ...
        const updatedItems = feedItems.map(i => {
            if (i.id === itemId) {
                const newCommentData = [...(i.commentData || []), newComment];
        ...
        try {
            await mockService.addCommentToFeedItem(itemId, newComment);
        } catch (error) {
