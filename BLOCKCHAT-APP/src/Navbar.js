...
const Navbar = ({ onBuyCreditsClick, credits, ownedNfts = [], unreadCounts = {} }) => {
    ...
    const isVip = isAdmin || ownedNfts.some(nft => nft.name === 'VIP Access NFT');
    ...
    const NavLinks = () => (
        <>
            ...
            {isVip && <NavLink to="/token-feed" style={({ isActive }) => isActive ? activeLink : navLink} onClick={() => setMobileMenuOpen(false)}>Token Feed</NavLink>}
            <NavLink to="/discover" style={({ isActive }) => isActive ? activeLink : navLink} onClick={() => setMobileMenuOpen(false)}>Discover</NavLink>
            ...
            <NavLink to={`/profile/${publicKey?.toBase58()}`} style={({ isActive }) => isActive ? activeLink : navLink} onClick={() => setMobileMenuOpen(false)}>Profile</NavLink>
