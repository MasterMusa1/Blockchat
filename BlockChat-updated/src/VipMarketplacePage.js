...
const VipMarketplacePage = () => {
    ...
    const handleBuy = async (nft) => {
        ...
        try {
            ...
            await dataService.buyVipNft(nft, publicKey.toBase58());

            const { user: updatedUser } = await dataService.getOrCreateUser(publicKey.toBase58());
            setUserData(updatedUser);

            setVipMarketplaceNfts(prev => prev.filter(item => item.id !== nft.id));
    ...
    const handleAdminDelete = async (nftId) => {
        if (!isAdmin) return;
        if (window.confirm("Are you sure you want to delete this VIP listing? This cannot be undone.")) {
            try {
                await dataService.deleteVipNft(nftId);
                setVipMarketplaceNfts(prev => prev.filter(nft => nft.id !== nftId));
        ...
        <div style={pageStyle}>
            ...
            <div style={headerStyle}>
                <div>
                    ...
                    <p style={{color: 'var(--text-secondary)', marginTop: '5px'}}>An exclusive marketplace for verified VIPs.</p>
                </div>
                <button onClick={() => setModalOpen(true)} style={listButtonStyle}>List Your NFT</button>
            </div>
            ...
            {vipMarketplaceNfts.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '50px 20px', color: 'var(--text-secondary)' }}>
                    ...
                    <p>Be the first to list an exclusive item!</p>
                </div>
            ) : (
                <div style={gridStyle}>
                    {vipMarketplaceNfts.map(nft => (
                        <div key={nft.id} style={nftCardStyle} ref={el => nftRefs.current[nft.id] = el}>
                            <button style={shareIconStyle} onClick={() => handleShare(nft.id)} title="Share NFT">🔗</button>
                            {isAdmin && (
                                <button style={adminDeleteIconStyle} onClick={() => handleAdminDelete(nft.id)} title="Admin: Delete Listing">🗑️</button>
                            )}
                            ...
                            <div style={nftInfoStyle}>
                                ...
                                <button
                                    style={{ ...buyButtonStyle, backgroundColor: loading === nft.id ? '#ccc' : 'var(--accent-color)' }}
                                    onClick={() => handleBuy(nft)}
                                    disabled={loading !== null || publicKey?.toBase58() === nft.seller}
                                >
                                    {loading === nft.id ? 'Processing...' : (publicKey?.toBase58() === nft.seller ? 'Your Listing' : 'Buy Now')}
                                </button>
                            </div>
                        </div>
                    ))}
